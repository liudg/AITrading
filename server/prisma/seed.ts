...
async function main() {
  ...
  const trade1 = await prisma.trade.create({
    data: {
      ...
      price: 450,
      amount: 22500,
      rationale: '英伟达在AI芯片领域占据主导地位，随着AI应用的爆发，需求将持续增长。技术指标显示突破关键阻力位，建议买入。',
      status: 'EXECUTED',
  ...
  const trade2 = await prisma.trade.create({
    data: {
      ...
      price: 250,
      amount: 5000,
      rationale: '特斯拉Q4交付量超预期，FSD技术取得重大突破，市场情绪积极，短期看涨。',
      status: 'EXECUTED',
  ...
  const trade3 = await prisma.trade.create({
    data: {
      ...
      price: 170,
      amount: 17000,
      rationale: '苹果新品发布会反响热烈，iPhone销量预期上调，加上服务业务增长强劲，值得配置。',
      status: 'EXECUTED',
  ...
  const trade4 = await prisma.trade.create({
    data: {
      ...
      price: 400,
      amount: 12000,
      rationale: '微软云业务Azure增长强劲，与OpenAI的合作深化，AI战略清晰，长期看好。',
      status: 'EXECUTED',
  ...
  const trade5 = await prisma.trade.create({
    data: {
      ...
      price: 140,
      amount: 21000,
      rationale: 'Google的Gemini模型表现优异，搜索业务稳定，广告收入回暖，估值合理。',
      status: 'EXECUTED',
  ...
  await prisma.reflection.create({
    data: {
      tradeId: trade7.id,
      modelId: deepseek.id,
      content: '这是一次成功的波段操作。买入时机准确，Meta刚刚发布了优秀的财报。持仓期间股价上涨了6.67%，获利了结的时机也比较合理。经验教训：科技股在财报季要特别关注，好的财报往往能带来短期爆发。',
      pnl: 600,
      score: 8,
    },
  });
  ...
  await prisma.stockPool.create({
    data: {
      ...
      createdBy: 'AI',
      reason: '这些是2024年最具潜力的科技股，涵盖了AI芯片、云计算、电动车、电商等热门赛道。',
      active: true,
    },
  });
  ...
  // 8. 创建市场数据
  console.log('创建市场数据...');
  const symbols = ['NVDA', 'TSLA', 'AAPL', 'MSFT', 'GOOGL', 'META', 'AMZN', 'AMD', 'NFLX', 'BABA'];
  ...
  const newsRecords = [
    {
      ...
      summary: '英伟达在GTC大会上发布了最新的Blackwell架构GPU，AI训练性能相比上一代提升3倍，引发市场热烈反响。',
      sentiment: 'POSITIVE',
      source: 'TechCrunch',
    ...
    },
    {
      ...
      summary: '特斯拉公布Q4交付数据，环比增长15%，超出华尔街预期。Model 3和Model Y销量强劲。',
      sentiment: 'POSITIVE',
      source: 'Reuters',
    ...
    },
    {
      ...
      summary: '产业链消息显示，iPhone 16 Pro系列预售火爆，苹果要求供应链提高产能以满足需求。',
      sentiment: 'POSITIVE',
      source: 'Bloomberg',
    ...
    },
    {
      ...
      summary: '微软财报显示Azure云服务同比增长30%，其中AI相关服务贡献了10个百分点的增长。',
      sentiment: 'POSITIVE',
      source: 'CNBC',
    ...
    },
    {
      ...
      summary: 'Google发布的Gemini Ultra模型在多项基准测试中超越OpenAI的GPT-4，AI竞争格局生变。',
      sentiment: 'POSITIVE',
      source: 'The Verge',
    ...
    },
    {
      ...
      summary: 'Meta Reality Labs部门本季度首次实现盈利，Quest 3头显销量强劲，VR生态日趋成熟。',
      sentiment: 'POSITIVE',
    ...
    },
    {
      ...
      summary: '美联储主席鲍威尔在新闻发布会上表示，如果通胀继续下行，明年可能考虑降息，科技股普遍上涨。',
      sentiment: 'POSITIVE',
    ...
    },
    {
      ...
      summary: 'AMD推出EPYC 9004系列处理器，性能和能效比均优于竞品，有望进一步扩大数据中心市场份额。',
      sentiment: 'POSITIVE',
      source: 'AnandTech',
    ...
    },
    {
      ...
      summary: 'Netflix热门剧集《鱿鱼游戏2》创下平台收视纪录，带动会员订阅大幅增长，股价走强。',
      sentiment: 'POSITIVE',
      source: 'Variety',
    ...
    },
    {
      ...
      summary: '阿里巴巴董事会批准了250亿美元的股票回购计划，显示管理层对公司长期发展的信心。',
      sentiment: 'POSITIVE',
